...
# 1. 페이지 설정
st.set_page_config(page_title="Broadcasting Master v991", layout="wide")
DB = 'stations.csv'
GS_URL = st.secrets.get("gsheets_url", "") # Secrets에 등록된 URL 사용
...
# [데이터 로드/저장 로직 - 시트 연동 기능 포함]
def load_db():
    df = pd.DataFrame(columns=CL, dtype=str)
    # 구글 시트 연동이 ON이고 URL이 있을 때
    if sd.get('gs_sync_on', False) and GS_URL:
        try:
            ...
            if not df_gs.empty:
                ...
                msg.success("🌐 구글 시트 데이터 로드 성공!")
                time.sleep(3)
                msg.empty()
                return df_gs.reindex(columns=CL, fill_value="")
        except: st.sidebar.error("❌ 구글 시트 로드 실패")

    # 연동 OFF이거나 시트 실패 시 로컬 로드
    try:
...
defaults = {
    ...
    'table_font_size': 26 # 표 글자 크기 초기값
}
...
st.markdown(f"""<style>
    ...
    /* 데이터프레임 폰트 동적 설정 */
    [data-testid="stDataFrame"] td, [data-testid="stDataFrame"] th {{ font-size: {sd.table_font_size}px !important; height: 45px !important; }}
...
with st.sidebar:
    ...
    if st.button("🎯 신규 위치 추출"):
        ...
        try:
            loc = Nominatim(user_agent="b_v991").reverse(f"{p[0]}, {p[1]}", timeout=3)
    ...
    if st.button("🎯 수정 위치 추출"):
        ...
        sd.map_key += 1; st.toast("🎯 조준경 위치 추출 완료!"); st.rerun()
    ...
    if sd.sel_reg != "전체":
        ...
        new_reg_name = st.text_input(f"'{sd.sel_reg}' 새 이름", key="bulk_name_input")
        if st.button("일괄 변경 실행"):
            if new_reg_name:
                sd.history.append(sd.df.copy())
...
for _, r in disp_df.iterrows():
    ...
    folium.Marker([lat, lon], icon=folium.Icon(color=color, icon='tower-broadcast', prefix='fa'), popup=r['이름']).add_to(m)
...
if not disp_df.empty:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.download_button("📥 현재 리스트 CSV 다운로드", data=disp_df.to_csv(index=False, encoding='utf-8-sig'), file_name="stations.csv", use_container_width=True)
    with c2:
