...
# 1. 페이지 설정
st.set_page_config(page_title="Broadcasting Master v940", layout="wide")
DB = 'stations.csv'
sd = st.session_state
...
def load_db(use_gsheets):
    if use_gsheets and HAS_GSHEETS:
        try:
            ...
            return df
        except: st.error("구글 시트 로드 실패. CSV를 엽니다.")
    try:
...
with st.sidebar:
    ...
    if new_gs != sd.use_gsheets:
        sd.use_gsheets = new_gs; sd.df = load_db(sd.use_gsheets); st.rerun()
        ...
        if st.button("🔍 검색") and s_addr:
            try:
                if ',' in s_addr: lat, lon = map(float, s_addr.split(',')); sd.base_center, sd.ref_loc = [lat, lon], [lat, lon]
                    ...
                    loc = Nominatim(user_agent="b_v940").geocode(s_addr)
