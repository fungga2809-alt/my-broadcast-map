...
with st.sidebar:
    ...
    with c_loc[0]:
        if st.button("📍검색"):
            if search_addr:
                try:
                    geolocator = Nominatim(user_agent="broadcasting_v165")
    ...
    if sd.m_mode == "신규 등록":
        ...
        final_reg = st.text_input("새 지역 명칭 입력", "") if sel_reg_name == "+ 직접 입력(새 지역)" else sel_reg_name
...
map_data = st_folium(m, width="100%", height=700, key=f"map_v165_{sd.map_key}")
