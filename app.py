...
# [수정] 롤백 방지를 위해 렌더링 센터와 패닝 센터를 분리
defaults = {
    'render_center': [35.1796, 129.0756], # 지도를 그릴 때 사용하는 고정 중심점
    'pan_center': [35.1796, 129.0756],    # 사용자가 드래그해서 움직인 실시간 중심점
    'render_zoom': 14, 't_la': None, 't_lo': None, 
...
with st.sidebar:
    ...
    geolocator = Nominatim(user_agent="broadcasting_v300")
        ...
        if st.button("📍검색"):
            if search_addr:
                try:
                    ...
                    if loc:
                        # 명시적 이동 시에만 render_center 업데이트 및 map_key 변경
                        sd.render_center = [loc.latitude, loc.longitude]
                        sd.pan_center = [loc.latitude, loc.longitude]
    ...
    with c_loc[1]:
        if st.button("🎯위치"):
            ...
            if gps and 'coords' in gps:
                ...
                sd.render_center, sd.pan_center = p, p
                sd.t_la, sd.t_lo = p[0], p[1]
                try:
    ...
    # [핵심] 조준경 위치 획득 버튼 - 드래그된 pan_center를 가져옴
    if st.button("🎯 지도 중앙을 등록 위치로 지정", type="primary"):
        sd.t_la, sd.t_lo = sd.pan_center[0], sd.pan_center[1]
        try:
    ...
    cur_la = sd.t_la if sd.t_la is not None else sd.render_center[0]
    cur_lo = sd.t_lo if sd.t_lo is not None else sd.render_center[1]
...
with map_container:
    ...
    """
    # [수정] 지도를 그릴 때는 항상 고정된 render_center를 사용합니다. (롤백 방지)
    m = folium.Map(location=sd.render_center, zoom_start=sd.render_zoom, tiles='https://mt1.google.com/vt/lyrs=y&hl=ko&x={x}&y={y}&z={z}', attr='G')
    ...
    # [수정] 헷갈리는 녹색 신규 마커 표시 로직을 완전히 삭제했습니다.

    # [수정] 지도 세로 높이를 850으로 확장
    map_data = st_folium(
        m, 
        use_container_width=True, 
        height=850, 
        key=f"map_v300_{sd.map_key}",
        returned_objects=["center", "zoom"]
    )

# [핵심] 지도 패닝(드래그) 시 pan_center만 조용히 업데이트합니다. (지도를 다시 그리지 않음)
if map_data:
    if map_data.get("center"): sd.pan_center = [map_data["center"]["lat"], map_data["center"]["lng"]]
    if map_data.get("zoom"): sd.render_zoom = map_data["zoom"]
...
if event and event.get("selection", {}).get("rows"):
    ...
    if sd.target_nm != sel['이름']:
        sd.render_center, sd.pan_center = [float(sel['위도']), float(sel['경도'])], [float(sel['위도']), float(sel['경도'])]
