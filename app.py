...
# 1. 페이지 설정
st.set_page_config(page_title="Broadcasting Master v968", layout="wide")
DB = 'stations.csv'
sd = st.session_state
...
with st.sidebar:
        ...
        if st.button("🔍 검색") and s_addr:
            try:
                if ',' in s_addr: lat, lon = map(float, s_addr.split(',')); sd.base_center, sd.ref_loc = [lat, lon], [lat, lon]
                    ...
                    loc = Nominatim(user_agent="b_v968").geocode(s_addr)
    ...
    if st.button("✅ 데이터 저장"):
        ...
        f_nm = sd.get('in_v_nm', "")
        if sd.m_mode == "신규 등록":
            f_reg = sd.get('in_reg_direct', "") if sd.get('in_reg_box') == "+ 직접 입력" else sd.get('in_reg_box')
        else:
            f_reg = sd.get('in_reg_direct', "")
    ...
    if sd.m_mode == "신규 등록":
        ...
        if curr_names:
            ...
            st.text_input("시설명 수정", key="in_v_nm")
            st.text_input("지역 수정", key="in_reg_direct") 
            st.radio("구분 수정", ["송신소", "중계소"], key="in_v_cat", horizontal=True)
