...
# 1. 페이지 설정 및 세션 초기화
st.set_page_config(page_title="Broadcasting Master v670", layout="wide")
DB = 'stations.csv'
sd = st.session_state
...
# 필수 변수 초기화
defaults = {
    'base_center': [35.1796, 129.0756], 'base_zoom': 14, 'map_key': 9000,
...
with st.sidebar:
    st.header("🔍 위치 제어")
    s_addr = st.text_input("주소/건물명 검색", key="search_top")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("🔍 검색") and s_addr:
            loc = Nominatim(user_agent="b_v670").geocode(s_addr)
    ...
    # 🔥 [핵심 추가] 지역 명칭 일괄 변경 도구
    with st.expander("🏷️ 지역 명칭 일괄 변경"):
        old_reg_name = st.selectbox("바꿀 대상", regs)
        new_reg_name = st.text_input("새 이름 (예: 서울)", value="서울")
        if st.button("🚀 명칭 일괄 변경 실행"):
            sd.history.append(sd.df.copy())
            sd.df['지역'] = sd.df['지역'].replace(old_reg_name, new_reg_name)
            sd.df.to_csv(DB, index=False, encoding='utf-8-sig')
            st.success(f"'{old_reg_name}'이(가) '{new_reg_name}'(으)로 모두 변경되었습니다.")
            st.rerun()
...
with st.container():
    ...
    for _, r in disp_df.iterrows():
        ...
        # 2단 팝업 구성
        dtv_list = "".join([f"<div style='display:flex; justify-content:space-between; margin-bottom:2px;'><b>{s}</b>: {r[s]}</div>" for s in SL_DTV if r[s]])
        uhd_list = "".join([f"<div style='display:flex; justify-content:space-between; margin-bottom:2px; color:#007bff;'><b>{s}</b>: {r[s]}</div>" for s in SL_UHD if r[s]])
        ...
        <div style='width:350px; font-family:sans-serif; font-size:15px; line-height:1.5;'>
            ...
            <div style='display:flex; justify-content:space-between;'>
                ...
                <div style='width:48%; border-left:1px solid #ddd; padding-left:10px;'><div style='font-weight:bold; border-bottom:1px solid #ddd; margin-bottom:5px; color:#007bff;'>✨ UHD</div>{uhd_list}</div>
            </div>
        </div>
        """
