...
with st.sidebar:
    ...
    if st.button("📍 위치 검색"):
        if search_addr:
            try:
                geolocator = Nominatim(user_agent="broadcasting_master_v115")
...
for _, r in disp_df.iterrows():
    try:
        ...
        # [v70 핵심] 지도 위 채널 팝업 내용 생성
        dtv_txt = " | ".join([f"{s}:{r[s]}" for s in SL_DTV])
        uhd_txt = " | ".join([f"{s}:{r[s]}" for s in SL_UHD])
        popup_html = f"<div style='width:250px;'><b>[{r['구분']}] {r['이름']}</b><br>DTV: {dtv_txt}<br>UHD: {uhd_txt}</div>"
        
        # 이름표 (디자인은 유지하되 클릭은 통과시킴)
...
# 지도 출력 및 클릭 감지
map_data = st_folium(m, width="100%", height=700, key=f"map_v115_{sd.map_key}")

# [v115 클릭 로직: v70처럼 민감하게 반응]
if map_data.get("last_object_clicked"): # 마커 클릭 시
...
elif map_data.get("last_clicked"): # 지도 배경 클릭 시
