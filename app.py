...
def get_google_format(lat, lon):
    def to_dms(deg, is_lat):import streamlit as st
import pandas as pd
import folium
from streamlit_folium import st_folium
import os
...
with st.sidebar:
    ...
    with c_loc[0]:
        if st.button("📍검색"):
            if search_addr:
                try:
                    geolocator = Nominatim(user_agent="broadcasting_v185")
    ...
    if sd.m_mode in ["신규 등록", "정보 수정"]:
        ...
        # [수정] 채널 설정 디자인 복구 (JSON 리스트 출력 제거)
        st.info("📺 물리 채널 설정")
        st.markdown("**📡 DTV 채널**")
        d_cols = st.columns(3)
...
map_data = st_folium(m, width="100%", height=700, key=f"map_v185_{sd.map_key}")
