...
defaults = {
    ...
    'btn_order': ["🔍 검색", "🧭 내 위치", "↩️ 복구"] # [신규] 기본 버튼 순서
}
...
st.markdown("""
    ...
    [data-testid="stSidebar"] { background-color: #eff2f6 !important; }
    ...
    [data-testid="stSidebar"] [data-testid="stHorizontalBlock"]:first-of-type button {
        ...
        background-color: #ffffff !important; border: 1px solid #c6ccd2 !important;
        border-radius: 6px !important; color: #31333f !important;
...
with st.sidebar:
    ...
    # [핵심] 사용자가 설정한 순서대로 버튼을 동적으로 생성
    c_loc = st.columns(len(sd.btn_order))
    geolocator = Nominatim(user_agent="broadcasting_v420")

    for idx, btn_name in enumerate(sd.btn_order):
        with c_loc[idx]:
            if btn_name == "🔍 검색":
                if st.button("🔍 검색"):
                    if search_addr:
    ...
    if st.button("🎯 지도 중앙을 수정 위치로 지정"):
        sd.m_mode = "정보 수정"; target_loc = sd.crosshair_center if sd.crosshair_center else sd.base_center
        sd.in_t_la, sd.in_t_lo, sd.base_center = target_loc[0], target_loc[1], [target_loc[0], target_loc[1]]
        try:
            rev = geolocator.reverse(f"{sd.in_t_la}, {sd.in_t_lo}")
            if rev: sd.in_v_addr = clean_kr_address(rev.address)
    ...
    if sd.m_mode in ["신규 등록", "정보 수정"]:
        ...
        if st.button("✅ 데이터 등록"):
            ...
            if sd.in_v_nm and final_reg:
                ...
                sd.in_v_nm, sd.in_v_addr, sd.last_loaded_nm = "", "", None; st.success("등록 완료!"); st.rerun()
    ...
    if sd.m_mode == "신규 등록":
        ...
        if names:
            ...
            c_la, c_lo = st.columns(2)
            c_la.number_input("위도(Dec)", format="%.6f", key="in_t_la"); c_lo.number_input("경도(Dec)", format="%.6f", key="in_t_lo")
    ...
    # =========================================================
    # 🔥 [신규 설정 메뉴] 사이드바 맨 하단에 배치
    # =========================================================
    st.divider()
    with st.expander("🎨 UI 레이아웃 설정"):
        st.write("상단 위치 제어 버튼의 순서를 변경합니다.")
        new_order = st.multiselect(
            "버튼 배치 순서 (드래그하여 변경)", 
            options=["🔍 검색", "🧭 내 위치", "↩️ 복구"],
            default=sd.btn_order
        )
        if new_order and len(new_order) == 3:
            if new_order != sd.btn_order:
                sd.btn_order = new_order
                st.rerun()
...
st.dataframe(disp_df[['지역', '구분', '이름'] + SL + ['위도', '경도', '주소']], use_container_width=True, hide_index=True)
