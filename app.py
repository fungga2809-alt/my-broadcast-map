...
with st.sidebar:
    ...
    st.subheader("🔍 위치 제어")
    search_addr = st.text_input("주소/지명 검색", key="addr_input")
    if st.button("📍 위치 검색"):
        if search_addr:
            try:
                geolocator = Nominatim(user_agent="broadcasting_v135")
    ...
    st.text_input("시설 명칭 (이름)", key="v_nm")
    
    # 좌표 입력 (v122 방식 - 숫자를 직접 고쳐도 마커가 따라옴)
...
# 지도가 리셋되며 마커를 확실히 그리는 v122 방식
map_data = st_folium(m, width="100%", height=700, key=f"map_v122_pure_{sd.map_key}")
...
if event and event.get("selection", {}).get("rows"):
    ...
    if sd.target_nm != sel_row['이름']:
        ...
        sd.t_la, sd.t_lo = float(sel_row['위도']), float(sel_row['경도']) # 표 클릭 시 좌표도 동기화
        sd.map_key += 1; st.rerun()

st.download_button("📥 백업 (CSV)", data=sd.df.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'), file_name='stations.csv')
