...
with st.sidebar:
    ...
    if sd.m_mode == "정보 수정" and names:
        ...
        if sd.last_loaded_nm != sd.target_nm:
            ...
            # 수정 모드에서 별도 클릭 좌표가 없으면 시설 좌표 사용
    ...
    # 좌표 입력 (지도의 t_la, t_lo와 실시간 연동)
    cur_la = float(sd.t_la if sd.t_la else sd.center[0])
    cur_lo = float(sd.t_lo if sd.t_lo else sd.center[1])
    la_v = st.number_input("위도", value=cur_la, format="%.6f", key="inp_la")
    lo_v = st.number_input("경도", value=cur_lo, format="%.6f", key="inp_lo")
    ...
    if st.button("✅ 데이터 저장"):
        if sd["v_nm"]:
            ...
            v = [sd["v_reg"], sd["v_cat"], sd["v_nm"]] + ch_data + [str(sd.t_la), str(sd.t_lo), ""]
...
for _, r in disp_df.iterrows():
    try:
        ...
        p_html = f"<div style='width:250px;'><b>[{r['구분']}] {r['이름']}</b><br>DTV: {dt_t}<br>UHD: {uh_t}</div>"
        ...
        # 마커 추가 (클릭 시 팝업만 발생)
        folium.Marker(p, icon=folium.Icon(color=color, icon='tower-broadcast', prefix='fa'), 
                      popup=folium.Popup(p_html, max_width=300)).add_to(m)
    except: pass
...
# 지도 출력 및 클릭 감지
map_data = st_folium(m, width="100%", height=700, key=f"map_v117_{sd.map_key}")

# [v70 순정 클릭 로직] 지도의 어디든 클릭하면 녹색 마커가 즉시 그 자리로 이동
if map_data.get("last_clicked"):
    cla, clo = map_data["last_clicked"]["lat"], map_data["last_clicked"]["lng"]
    if sd.t_la != cla:
        sd.t_la, sd.t_lo = cla, clo
        sd.m_mode, sd.target_nm = "새로 등록", None # 클릭 시 신규 등록 모드 전환
        sd.map_key += 1; st.rerun()

st.divider()
# [하단 표] v82 스타일 (중앙 정렬 + 색상)
...
st.download_button(label="📥 전체 CSV 백업", data=sd.df.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'), file_name='stations.csv')
