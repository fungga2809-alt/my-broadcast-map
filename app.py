...
st.set_page_config(page_title="Broadcasting Master v660", layout="wide")
DB = 'stations.csv'
sd = st.session_state
...
# 🔥 [핵심] 모든 필수 변수를 안전하게 초기화 (AttributeError 해결)
defaults = {
...
with st.sidebar:
    ...
    with c1:
        if st.button("🔍 검색") and s_addr:
            loc = Nominatim(user_agent="b_v660").geocode(s_addr)
...
with st.container():
    ...
    for _, r in disp_df.iterrows():
        ...
        # 🔥 [핵심] 2단 컬럼 팝업 (image_0b2222.png 재현)
        ...
        <div style='width:350px; font-family:sans-serif; font-size:15px; line-height:1.5;'>
            ...
            <div style='display:flex; justify-content:space-between;'>
                <div style='width:48%;'>
                    <div style='font-weight:bold; border-bottom:1px solid #ddd; margin-bottom:5px;'>📡 DTV 채널</div>
                    {dtv_list}
                </div>
                <div style='width:48%; border-left:1px solid #ddd; padding-left:10px;'>
                    <div style='font-weight:bold; border-bottom:1px solid #ddd; margin-bottom:5px; color:#007bff;'>✨ UHD 채널</div>
                    {uhd_list}
                </div>
            </div>
        </div>
