...
# 1. 페이지 설정 및 초기화
st.set_page_config(page_title="Broadcasting Master v700", layout="wide")
DB = 'stations.csv'
sd = st.session_state
...
# 필수 변수 안전 초기화
defaults = {
    'base_center': [35.1796, 129.0756], 'base_zoom': 14, 'map_key': 15000,
...
with st.sidebar:
    # 1. 위치 제어 (검색/복구/좌표복사)
    st.header("🔍 위치 제어")
    s_addr = st.text_input("주소 또는 좌표 검색", placeholder="예: 부산시청 또는 35.17, 129.07")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("🔍 검색") and s_addr:
            try:
                ...
                if ',' in s_addr:
                    ...
                    loc = Nominatim(user_agent="b_v700").geocode(s_addr)
...
with st.container():
    ...
    for _, r in disp_df.iterrows():
        ...
        p_html = f"<div style='width:350px; font-family:sans-serif; font-size:15px; line-height:1.5;'><div style='font-size:20px; font-weight:bold; color:#333; border-bottom:2px solid #ccc; padding-bottom:5px; margin-bottom:10px;'>[{r['구분']}] <span style='background-color:#ffff00; padding:2px 5px;'>{r['이름']}</span></div><div style='color:#666; margin-bottom:12px; font-size:13px;'>{r['주소']}</div><div style='display:flex; justify-content:space-between;'><div style='width:48%;'><div style='font-weight:bold; border-bottom:1px solid #ddd; margin-bottom:5px;'>📡 DTV 채널</div>{dtv_list}</div><div style='width:48%; border-left:1px solid #ddd; padding-left:12px;'><div style='font-weight:bold; border-bottom:1px solid #ddd; margin-bottom:5px; color:#007bff;'>✨ UHD 채널</div>{uhd_list}</div></div></div>"
...
# 🔥 [핵심 추가] 데이터 현황 (위도/경도를 구글어스 좌표로 변환하여 출력)
st.subheader("📊 데이터 현황")
...
if not disp_df.empty:
    ...
    # 🔥 위도, 경도 열을 구글어스 DMS 좌표 열 하나로 통합
    view_df['구글어스 좌표'] = view_df.apply(lambda x: get_google_format(x['위도'], x['경도']), axis=1)
    # 기존 위도/경도 열은 숨기고 가독성 좋은 순서로 재배치
    display_cols = ['지역', '구분', '이름'] + SL + ['구글어스 좌표', '주소']
